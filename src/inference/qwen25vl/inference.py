...
from transformers import Qwen2_5_VLForConditionalGeneration, AutoTokenizer, AutoProcessor       # transformers>=4.51.3
...
from src.qwen_vl.get_prompt_new_after_rm import get_prompt
...
if __name__=='__main__':
    ...
    with open(out_file, 'w') as fin:
        for step, batch in tqdm(enumerate(gendata_reader)):
            with torch.no_grad():
                inputs = batch['inputs']
                # index=int(batch['qids'][-1].split('_')[-1])
                # if index>288:
                #     break
