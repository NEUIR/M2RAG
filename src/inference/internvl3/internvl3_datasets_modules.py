...
def load_trec_data(trec_path, topk=5):
    assert topk <= 10 and topk >= 1, "topk must be in [1,10]"
    qid_2_candids={}
...
def load_clueweb_candi_image_map(idx_map_path):
    candi_map = {}
    with jsonlines.open(idx_map_path) as reader:
        for obj in reader:
            candi_map[obj['id']] = obj['image_path']
    return candi_map
...
def load_data(image_path, trec_path, data_path, idx_map_path=None, topk=5):   
    data=[]
    qid_2_candids = load_trec_data(trec_path, topk=topk)
    if idx_map_path!=None:
        idx_map = load_clueweb_candi_image_map(idx_map_path)
    else:
        idx_map = None
    with open(data_path) as fin:
        for line in fin:
            ...
            for img_id in qid_2_candids[id]:
                ...
                elif 'clueweb' in image_path and idx_map!=None:
                    image_name = idx_map[img_id].split('/')[-1]
                    image = os.path.join(image_path, f'qid_{id}',image_name)
                one = {
                    'id':id,
                    'image':image,
                    'conversations':[
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx, j in enumerate(range(0, len(sources), 2)):
            ...
            if DEFAULT_IMAGE_TOKEN in user_input or DEFAULT_VIDEO_TOKEN in user_input or LLAVA_IMAGE_TOKEN in user_input or INTERNVL_IMAGE_TOKEN in user_input:
